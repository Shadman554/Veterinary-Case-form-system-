...
import { Search, Calendar, SlidersHorizontal, PawPrint, User, Phone, Stethoscope, AlertTriangle } from 'lucide-react'
          ...
          <div className="space-y-2">
            {filtered.map((c) => (
              <Card key={c.id}>
                <button
                  type="button"
                  className="grid w-full grid-cols-[88px_1fr_1fr_1fr] items-center gap-4 px-5 py-4 text-left transition hover:bg-input/40"
                >
                  {/* Col 1 — ID + date */}
                  <div>
                    ...
                    {c.stray && (
                      <span className="mt-1.5 inline-flex items-center gap-1 rounded-md border border-warning/30 bg-warning-soft px-1.5 py-0.5 text-[10px] font-bold text-warning">
                  ...
                  {/* Col 4 — Diagnosis */}
                  <div className="min-w-0">
                    <span className="text-sm text-foreground">{c.diagnosis}</span>
                    {(c as any).followUp && (
                      <div className="mt-1 text-xs text-primary">📅 Follow-up: {(c as any).followUp}</div>
                    )}
                  </div>
                </button>
              </Card>
