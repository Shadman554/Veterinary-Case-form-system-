...
function CaseCard({ c }: { c: FUCase }) {
  const isOverdue = c.overdueDays > 0
  const isToday   = c.overdueDays === 0

  return (
    <Card>
      <div className={`overflow-hidden rounded-2xl border-l-4 ${isOverdue ? 'border-danger' : isToday ? 'border-warning' : 'border-primary'}`}>
        <div className="grid grid-cols-[1fr_auto] gap-4 p-5">
          <div className="min-w-0">
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              {isOverdue && (
                <span className="rounded-full bg-danger-soft px-2 py-0.5 text-[11px] font-bold text-danger">{c.overdueDays}d overdue</span>
              )}
              {isToday && (
                <span className="rounded-full bg-warning-soft px-2 py-0.5 text-[11px] font-bold text-warning">Due today</span>
              )}
            </div>
            ...
            <div className="mb-4 rounded-xl border border-border bg-input px-3.5 py-2.5 text-sm text-muted">
              <span className="font-semibold text-foreground">Diagnosis: </span>{c.diagnosis}
            </div>

            <div className="flex flex-wrap gap-2">
              <a href={`tel:${c.phone}`} className="flex items-center gap-1.5 rounded-xl border border-primary/30 bg-primary/10 px-3.5 py-2 text-xs font-semibold text-primary transition hover:bg-primary/20">
          ...
          {/* Date badge */}
          <div className={`flex flex-col items-center justify-center rounded-xl px-4 py-3 text-center ${
            isOverdue ? 'bg-danger-soft text-danger' : isToday ? 'bg-warning-soft text-warning' : 'bg-primary/10 text-primary'
          }`}>
...
function GroupHeader({ icon: Icon, label, count, color }: {
  icon: typeof CircleAlert; label: string; count: number; color: string
}) {
  return (
    <div className="mb-3 flex items-center gap-2">
      ...
      <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${
        color === 'text-danger' ? 'bg-danger-soft text-danger' :
        color === 'text-warning' ? 'bg-warning-soft text-warning' :
...
export default function FollowUpPage() {
  return (
    <main className="scroll-thin flex-1 overflow-y-auto px-6 py-6">
      ...
      <div className="mb-6">
        <GroupHeader icon={CircleAlert} label="OVERDUE" count={OVERDUE.length} color="text-danger" />
        <div className="space-y-2">{OVERDUE.map((c) => <CaseCard key={c.caseId} c={c} />)}</div>
      </div>

      <div className="mb-6">
        <GroupHeader icon={Clock} label="DUE TODAY" count={DUE_TODAY.length} color="text-warning" />
        <div className="space-y-2">{DUE_TODAY.map((c) => <CaseCard key={c.caseId} c={c} />)}</div>
      </div>

      <div className="mb-6">
        <GroupHeader icon={Calendar} label="UPCOMING" count={UPCOMING.length} color="text-primary" />
